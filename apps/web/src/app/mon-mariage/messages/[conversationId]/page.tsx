...
import { joinConversationAction, sendMessageAction } from "@/lib/messaging-actions";
...
export default async function ConversationPage({
  params,
}: PageProps<"/mon-mariage/messages/[conversationId]">) {
  ...
  const returnPath = `/mon-mariage/messages/${conversationId}`;

  if (!myMembership) {
    return (
      <main className="mx-auto max-w-lg px-6 py-24 text-center">
        <h1 className="mb-4 font-[family-name:var(--font-display)] text-2xl italic text-ink">
          {vendor?.name ?? "Conversation"}
        </h1>
        <p className="mb-6 text-sm text-ink-soft">
          Cette conversation existe déjà pour votre mariage, mais vous n&apos;y avez pas encore accès en tant que
          participant.
        </p>
        <form action={joinConversationAction}>
